...
export function buildDamageAnalysisPrompt(params: {
  ...
  const t1 = cfg.labourRates.tier1
  const t2 = cfg.labourRates.tier2
  const t3 = cfg.labourRates.tier3

  const depSchedule = cfg.depreciation.metalGlassOther
    .map((d, i) => {
      if (i === 0) return `  - 0–6 months: ${d.rate}%`
      if (i === 1) return `  - 6m–1yr: ${d.rate}%`
      if (i === 2) return `  - 1–2yr: ${d.rate}%`
      if (i === 3) return `  - 2–3yr: ${d.rate}%`
      if (i === 4) return `  - 3–4yr: ${d.rate}%`
      if (i === 5) return `  - 4–5yr: ${d.rate}%`
      return `  - >5yr: ${d.rate}%`
    })
    .join('\n')

  const s = cfg.settlement
  const deductible = vehicleType === 'two_wheeler'
    ? s.compulsoryDeductible.twoWheeler
    : vehicleType === 'commercial'
      ? s.compulsoryDeductible.commercial
      : (engineCc || 1200) >= 1500
        ? s.compulsoryDeductible.carLargeEngine
        : s.compulsoryDeductible.carSmallEngine
...
- Incident Type: ${incidentType}
- Zero Depreciation Add-on: ${zeroDep ? 'YES' : 'NO'}
- Max claim limit for auto-settlement: ₹${s.autoApprovalLimit.toLocaleString('en-IN')}

IRDA DEPRECIATION SCHEDULE (apply unless zero-dep is YES):
- Rubber/Nylon/Plastic parts: ${cfg.depreciation.rubberPlasticNylon}% always
- All other parts (metal, glass etc.):
${depSchedule}

INDIAN LABOUR RATES (${city} / ${cityTier}):
- Tier 1 cities (${t1.label}): ₹${t1.min}–${t1.max}/hour
- Tier 2 cities: ₹${t2.min}–${t2.max}/hour
- Tier 3 cities: ₹${t3.min}–${t3.max}/hour

COMPULSORY DEDUCTIBLE (non-waivable): ₹${deductible.toLocaleString('en-IN')}
...
4. Calculate labour hours and labour cost at the appropriate tier rate
5. Apply IRDA depreciation schedule (skip if zero_dep=YES)
6. Check for fraud indicators: ${cfg.fraudSignals.join(', ')}
7. Overall confidence score 0-100 based on photo quality, consistency, and certainty of assessment
8. Recommendation: "auto_approve" if final_claim_amount ≤ ₹${s.autoApprovalLimit.toLocaleString('en-IN')} AND fraud_score ≤ ${s.maxFraudScoreForApproval} AND confidence ≥ ${s.minConfidenceForApproval}, otherwise "manual_review", "reject", or "escalate"
...
OUTPUT FORMAT — Respond ONLY with valid JSON, no explanation, no markdown, no extra text:
{
  ...
  "total_estimate": {
    ...
    "final_claim_amount": 9400,
    "within_limit": true,
    "limit_check": "₹9,400 < ₹${s.autoApprovalLimit.toLocaleString('en-IN')} ✅"
  },
  "fraud_indicators": [
    {
      "type": "prior_damage",
