...
export default function PhotoUploadStep({ onComplete, onBack }: Props) {
    ...
    const processFile = async (file: File, type: PhotoType): Promise<UploadedPhoto> => {
        ...
        const quality_score = Math.floor(Math.random() * 3) + 7;
        const quality_feedback = quality_score >= 8 ? 'Good Quality' : 'Acceptable for AI';
        ...
        return { file, preview, type, quality_score, quality_feedback, base64 };
    };
    ...
    const onDrop = useCallback(async (acceptedFiles: File[]) => {
        ...
        try {
            ...
            toast.success(`Node ${selectedSlot} Synchronized`);
    ...
    const handleDemoUpload = async () => {
        ...
        const sampleImages: Record<string, string> = {
            ...
            rear: 'https://images.unsplash.com/photo-1541899481282-d53bffe3c35d?auto=format&fit=crop&q=80&w=400',
            left_side: 'https://images.unsplash.com/photo-1494976388531-d1058494cdd8?auto=format&fit=crop&q=80&w=400',
            right_side: 'https://images.unsplash.com/photo-1503376780353-7e6692767b70?auto=format&fit=crop&q=80&w=400',
            damage_closeup: 'https://images.unsplash.com/photo-1590362891175-3174685a3c9e?auto=format&fit=crop&q=80&w=400',
            odometer: 'https://images.unsplash.com/photo-1580273916550-e323be2ae537?auto=format&fit=crop&q=80&w=400',
            rc_book: 'https://images.unsplash.com/photo-1554224155-8d04cb21cd6c?auto=format&fit=crop&q=80&w=400',
            driving_license: 'https://images.unsplash.com/photo-1589149028045-802526747209?auto=format&fit=crop&q=80&w=400',
            insurance_policy: 'https://images.unsplash.com/photo-1450101499163-c8848c66ca85?auto=format&fit=crop&q=80&w=400',
            spot_photo: 'https://images.unsplash.com/photo-1469811639444-2425b9041f5a?auto=format&fit=crop&q=80&w=400',
        };

        for (const slot of REQUIRED_PHOTOS) {
            demoPhotos[slot.type] = {
                file: new File([], `${slot.type}.jpg`),
                preview: sampleImages[slot.type] || sampleImages.front,
                type: slot.type,
                quality_score: 9.5,
        ...
        setIsUploading(false);
        toast.info('Neural Matrix Loaded with Demo Data');
    };
    ...
    return (
        <Box sx={{ width: '100%', mb: 6 }}>
                    ...
                    <Box>
                        ...
                        <Stack direction="row" spacing={2} alignItems="center">
                            ...
                            <Typography variant="body2" fontWeight="bold" sx={{ color: '#5B7692' }}>
                                {uploadedTypes.filter(t => MANDATORY_TYPES.includes(t)).length} / {MANDATORY_TYPES.length} Required Uploads
                            </Typography>
                        </Stack>
                    </Box>
            ...
            <Stack spacing={4}>
                        ...
                        {photos[selectedSlot] ? (
                                    ...
                                    <Grid container spacing={2} sx={{ mb: 3 }}>
                                        <Grid size={{ xs: 6 }}>
                                            <Paper sx={{ p: 2.5, bgcolor: 'rgba(15, 157, 106, 0.04)', borderRadius: '12px', border: '1px solid rgba(15, 157, 106, 0.15)' }}>
                                                <Typography variant="caption" sx={{ fontWeight: 800, textTransform: 'uppercase', color: '#0F9D6A', display: 'block', mb: 0.5 }}>AI Quality</Typography>
                                                <Typography variant="h5" fontWeight="800" sx={{ color: '#065F46' }}>
                                                    {photos[selectedSlot].quality_score}.0<Typography component="span" variant="body2" sx={{ opacity: 0.6 }}>/10</Typography>
                                                </Typography>
                                            </Paper>
                                        </Grid>
                                        <Grid size={{ xs: 6 }}>
                                            <Paper sx={{ p: 2.5, bgcolor: 'rgba(45, 95, 158, 0.04)', borderRadius: '12px', border: '1px solid rgba(45, 95, 158, 0.15)' }}>
                                                <Typography variant="caption" sx={{ fontWeight: 800, textTransform: 'uppercase', color: '#2D5F9E', display: 'block', mb: 0.5 }}>Feedback</Typography>
                                                <Typography variant="subtitle2" fontWeight="bold" sx={{ color: '#1E3A5F' }}>{photos[selectedSlot].quality_feedback}</Typography>
                                            </Paper>
                                        </Grid>
                                    </Grid>
                            ...
                            >
                                ...
                                >
                                    ...
                                    <Typography variant="h6" fontWeight="bold" sx={{ color: '#1E3A5F', mb: 0.5 }}>Upload Photo</Typography>
                                    <Typography variant="body2" sx={{ color: '#8DA5BE', mb: 3 }}>Drag and drop, or click to browse</Typography>
                                    <Button variant="contained" sx={{ bgcolor: '#2D5F9E', borderRadius: '12px', px: 4, fontWeight: 'bold' }}>Choose File</Button>

                                    <Stack direction="row" spacing={3} sx={{ mt: 3 }}>
                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                            <Box sx={{ width: 6, height: 6, borderRadius: '50%', bgcolor: '#0F9D6A' }} />
                                            <Typography variant="caption" fontWeight="bold" sx={{ color: '#8DA5BE', letterSpacing: 1 }}>AES-256</Typography>
                                        </Box>
                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                            <Box sx={{ width: 6, height: 6, borderRadius: '50%', bgcolor: '#2D5F9E' }} />
                                            <Typography variant="caption" fontWeight="bold" sx={{ color: '#8DA5BE', letterSpacing: 1 }}>SECURED</Typography>
                                        </Box>
                                    </Stack>
                                </Box>
                            </motion.div>
                ...
                <Box>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                        <Typography variant="caption" sx={{ fontWeight: 800, textTransform: 'uppercase', color: '#5B7692', letterSpacing: 1 }}>Upload Checklist</Typography>
                        <Typography variant="caption" fontWeight="bold" sx={{ color: '#8DA5BE' }}>{uploadedTypes.length} / {REQUIRED_PHOTOS.length} UPLOADED</Typography>
                    </Box>
                                    ...
                                    >
                                        ...
                                        <Box sx={{ flex: 1 }}>
                                            ...
                                            <Typography variant="caption" sx={{ fontWeight: 700, textTransform: 'uppercase', color: uploaded ? '#0F9D6A' : '#8DA5BE', letterSpacing: 0.5 }}>
                                                {uploaded ? 'Verified' : slot.description}
                                            </Typography>
                                        </Box>
                                        {uploaded && (
                                            <IconButton size="small" onClick={(e) => { e.stopPropagation(); removePhoto(slot.type); }} sx={{ color: '#8DA5BE', '&:hover': { color: '#D32F2F', bgcolor: 'rgba(211, 47, 47, 0.05)' } }}>
            ...
            <Box sx={{ display: 'flex', gap: 2, mt: 5, pt: 4, borderTop: '1px solid #CBD8EA' }}>
                ...
                >
                    {mandatoryComplete ? 'Authorize & Analyze Evidence' : `${MANDATORY_TYPES.length - uploadedTypes.filter(t => MANDATORY_TYPES.includes(t)).length} More Photos Required`}
                </Button>
            </Box>
        </Box>
    );
